<template>
  <div
    class="pane-container relative overflow-scroll hide-scrollbar bg-secondary"
    border="4 secondary"
  >
    ...
    <div
      id="zoom-bar"
      class="hstack fixed z-20 bottom-4 ml-2 shadow-c rounded-full overflow-hidden text-primary-foreground bg-blue-500 pointer-events-auto"
      lg="bottom-auto top-15 opacity-0 hover:opacity-100 focus-within:opacity-100"
    >
      <button type="button" @click="scale *= 1.1" aria-label="Zoom in">
        <span i-lucide:zoom-in />
      </button>
      <button type="button" @click="scale /= 1.1" aria-label="Zoom out">
        <span i-lucide:zoom-out />
      </button>
      <button type="button" @click="fitWidth" aria-label="Fit width">
        <span i-fluent:arrow-autofit-width-20-filled />
      </button>
      <button type="button" @click="fitHeight" aria-label="Fit height">
